...
const benchmarkByPlatform: Record<string, { engagement: number; conversion: number }> = {
...
export const getSocialAnalytics = tool(
  async ({
    ...
    try {
      ...
      if (campaignId) {
        query._id = new mongoose.Types.ObjectId(campaignId);
      }
      ...
      if (instituteId) {
        postFilter.institute = new mongoose.Types.ObjectId(instituteId);
      }

      if (campaignId) {
        postFilter.campaign = new mongoose.Types.ObjectId(campaignId);
      }

      const recentPosts = await Post.find(postFilter)
      ...
      const campaignStats: Array<{
        ...
        .map((campaign: any) => campaign.stats)
        .filter(Boolean)
        .map((stats: any) => ({
      ...
      const avgEngagementRate = campaignStats.length
        ? campaignStats.reduce((sum: number, stat: { engagement: number }) => sum + stat.engagement, 0) / campaignStats.length
        : 0;

      const avgConversionRate = campaignStats.length
        ? campaignStats.reduce((sum: number, stat: { conversions: number }) => sum + stat.conversions, 0) / campaignStats.length
        : 0;

      const platformBenchmark = normalizedPlatform
        ? benchmarkByPlatform[normalizedPlatform] ?? { engagement: 4, conversion: 1.5 }
        : { engagement: 4.2, conversion: 1.6 };

      const topPerformingContent = recentPosts.slice(0, limit).map((post: any) => ({
        id: post._id?.toString?.() ?? "unknown",
        platform: post.platform,
        type: post.postType ?? "image",
        publishedAt: post.publishedAt,
        contentPreview: (post.text || "Untitled post").slice(0, 120),
        estimatedEngagementRate: Math.max(
          0,
          Number((platformBenchmark.engagement * (post.images?.length ? 1.1 : 1) * (post.videos?.length ? 1.2 : 1)).toFixed(2))
        ),
      ...
      return {
        success: true,
        platform: normalizedPlatform ?? "all",
        periodDays: days,
        summary: {
  ...
  },
  {
    ...
    description:
      "Get historical campaign and post performance data for a social platform, plus engagement insights and recommendations.",
    schema: z.object({
      platform: z
        .string()
        .optional()
